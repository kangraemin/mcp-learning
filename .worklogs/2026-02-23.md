...
### 작업 내용
- chore: pypi.txt를 .gitignore에 추가
- PyPI 토큰 파일이 실수로 커밋되지 않도록 제외.
...
### 요청사항
- PyPI README가 최신 설치 명령으로 반영되도록 재배포

### 작업 내용
- pyproject.toml 버전 0.1.0 → 0.1.1 bump
- `uv build && uv publish`로 v0.1.1 PyPI 재배포

### 작업 내용
- chore: v0.1.1 버전 bump — PyPI README 최신화
- uvx til-server 설치 명령이 PyPI 페이지에 반영되도록 재배포.

### 변경 파일
- `pyproject.toml`: 버전 0.1.1로 bump
...
### 요청사항
- skills.sh에서 이 프로젝트에 맞는 스킬/에이전트를 찾아서 로컬 설치

### 작업 내용
- anthropics/skills, wshobson/agents, obra/superpowers 3개 레포에서 스킬 설치
- 처음에 전역(-g) 설치했다가 사용자 요청으로 프로젝트 로컬로 재설치
- 전역 스킬 제거 후 `.agents/skills/`에 정착

### 변경 파일
- `.agents/skills/`: mcp-builder, webapp-testing, python-testing-patterns 등 스킬 설치
- `skills-lock.json`: 설치된 스킬 lock 파일 생성

### 토큰 사용량
- 모델: claude-sonnet-4-6
- 이번 작업: 17,589,895 토큰 / $8.24
- 소요 시간: 2분
- 일일 누적: 53,590,734 토큰 / $29.98
...
### 요청사항
- Notion 백엔드 추가 + 양방향 마이그레이션 기능 구현 시작 (/dev)

### 작업 내용
- DEVELOPMENT_GUIDE.md 및 docs/ 가이드 문서 생성 (신규 모드 init-project)
- til-dev 팀 생성 및 Lead 에이전트 스폰 (코드베이스 분석 중)

### 변경 파일
- `DEVELOPMENT_GUIDE.md`: 프로젝트 허브 가이드 생성
- `docs/ARCHITECTURE.md`: 현재 구조 및 목표 아키텍처 문서화
- `docs/CODING_CONVENTIONS.md`: 네이밍, 에러 처리, 백엔드 인터페이스 규칙
- `docs/TESTING.md`: 테스트 전략 및 검증 명령 정리

### 토큰 사용량
- 모델: claude-sonnet-4-6
- 이번 작업: 4,458,841 토큰 / $2.25
- 소요 시간: 4분
- 일일 누적: 58,049,575 토큰 / $32.23
